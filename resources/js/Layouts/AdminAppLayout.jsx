...
export default function AdminAppLayout({ children }) {
  ...
  const SidebarContent = ({ onClose }) => (
    <div className="flex flex-col h-full relative overflow-hidden" style={{ background: 'var(--sidebar-bg, inherit)' }}>
      ...
      >
        <div>
          ...
          <p className="text-xs pl-9" style={{ color: 'rgba(255,255,255,0.55)' }}>
            Manage platform
          </p>
        </div>
        {onClose && (
            ...
            >
              ...
              >
                <item.icon
                  ...
                  style={{ color: isActive ? 'var(--color-brand-primary, #6366f1)' : 'rgba(255,255,255,0.85)' }}
                />
              </span>
      ...
      >
        {/* User row */}
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl mb-2" style={{ background: 'rgba(255,255,255,0.08)' }}>
          <div
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs truncate" style={{ color: 'rgba(255,255,255,0.5)' }}>Administrator</p>
          </div>
        </div>
    ...
    <div className="min-h-screen flex bg-brand-background dark:bg-gray-900">
      ...
      <div className="md:hidden relative w-full flex flex-col">
        ...
        <header className="sticky top-0 z-20 bg-white dark:bg-gray-800 border-b border-brand-border dark:border-gray-700 px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex flex-col items-center">
            <h1 className="text-sm font-bold text-brand-primary dark:text-gray-100 leading-tight">{getPageName()}</h1>
            <span className="text-xs text-brand-secondary dark:text-gray-400">Admin Portal</span>
          </div>
      ...
      <div className="hidden md:flex w-full">
        ...
        <div className="flex-1 ml-64 flex flex-col overflow-hidden">
          {/* Top Bar — Desktop */}
          <header className="sticky top-0 z-20 bg-white dark:bg-gray-800 border-b border-brand-border dark:border-gray-700 px-6 py-0 flex items-center justify-between" style={{ minHeight: '64px' }}>

            {/* Left: Breadcrumb + Page title */}
            <div className="flex flex-col justify-center">
              <div className="flex items-center gap-1.5 mb-0.5">
                <span className="text-xs text-brand-secondary dark:text-gray-500 font-medium">Admin</span>
                <ChevronRight className="w-3 h-3 text-brand-secondary dark:text-gray-600" />
                <span className="text-xs text-brand-primary dark:text-gray-400 font-semibold">{getPageName()}</span>
              </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2.5 pl-1">
                <div className="flex flex-col items-end">
                  <span className="text-sm font-semibold text-brand-primary dark:text-gray-100 leading-tight">{user?.name || 'Admin'}</span>
                  <span className="text-xs text-brand-secondary dark:text-gray-500 leading-tight">Administrator</span>
                </div>
                <div className="w-9 h-9 rounded-xl bg-brand-primary flex items-center justify-center flex-shrink-0 overflow-hidden ring-2 ring-brand-primary/20">
                  {user?.profile_image ? (
                    <img src={`/assets/profile_images/${user.profile_image}`} alt={user?.name} className="w-full h-full object-cover" />

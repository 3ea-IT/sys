...
// CategoryDropdown
//
// The parent pill row uses `overflow-x-auto` so it can scroll horizontally.
// Per the CSS spec, once one axis is set to auto/hidden/scroll, the browser
// forces the *other* axis to `auto` as well — `overflow-y-visible` on that
// same element is silently ignored. That was clipping the dropdown menus for
// "Events" and "Tourism" the instant they tried to render below the row.
//
// Fix: render the menu through a React portal straight into document.body,
// positioned with `position: fixed` from the trigger's live bounding rect.
// That takes it completely outside the scrolling container, so it can never
// be clipped by it, and it still tracks scroll/resize.
// ─────────────────────────────────────────────────────────────────────────────
    ...
    return (
        <>
            <button
                ...
                ref={triggerRef}
                onClick={toggleOpen}
                className="flex items-center gap-2 flex-shrink-0 px-3.5 py-2 rounded-2xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 hover:border-gray-300 dark:hover:border-gray-600 transition-all group shadow-sm cursor-pointer"
                aria-haspopup="true"
                aria-expanded={open}
            >
            ...
            {open &&
                createPortal(
                    <div
                        ...
                        className="w-56 py-1.5 rounded-xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-lg"
                    >
        ...
        return (
            ...
            >
                ...
                <div className="p-3.5 flex-1 flex flex-col gap-1">
                    ...
                    <p className="text-[11px] text-gray-400 dark:text-gray-500 flex items-center gap-1">
                        <ClockIcon size={10} />{" "}
                        {formatDuration(movie.duration || 150)}
                    </p>
        ...
        <AppLayout>
            ...
            <section className="mt-8 md:mt-10">
                <div className="flex justify-between items-center mb-4 md:mb-5">
                    ...
                    <Link
                        href="/explore"
                        className="flex items-center gap-1 text-xs font-medium text-gray-400 dark:text-gray-500 hover:text-brand-primary transition-colors"
                    >

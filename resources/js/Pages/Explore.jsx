...
export default function Explore({
    ...
    const getEventStartDate = (exp) =>
        exp.start_date || exp.date || exp.created_at || null;
          ...
          >
            ...
            <section>
                <div className="flex items-center justify-between mb-4 md:mb-6">
                    <h2 className="text-sm md:text-base font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                        Featured Events
                    </h2>
                </div>
                ...
                <div className="flex gap-0 mb-4 overflow-x-auto no-scrollbar pb-2">
                    ...
                    <FilterButton
                        label="Premium (₹100+)"
                        active={activeFilter === "highPrice"}
                        onClick={() => setActiveFilter("highPrice")}
                    />
                    <FilterButton
                        label="Budget"
                        active={activeFilter === "lowPrice"}
                    ...
                    {filteredExperiences.length === 0 ? (
                        ...
                        filteredExperiences.map((exp, index) => (
                            <FeaturedCard
                                ...
                                price={
                                    exp.instant_price
                                        ? parseFloat(exp.instant_price)
                                        : parseFloat(exp.hold_token || 0)
                                }
...
function FeaturedCard({
    ...
    const isShowingInstantPrice =
        instant_price &&
        parseFloat(instant_price) > 0 &&
        parseFloat(instant_availability) > 0;
    const displayPrice = isShowingInstantPrice
        ? parseFloat(instant_price)
        : parseFloat(hold_token || price || 0);
    ...
    return (
        <div
            className={`bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden border border-gray-100 dark:border-gray-700 transition-all relative ${seats_full ? "opacity-50" : "hover:shadow-md"}`}
        >
            ...
            <Link
                ...
                onClick={(e) =>
                    seats_full && (e.preventDefault(), e.stopPropagation())
                }
            >
                ...
                <div className="p-4">
                    <h3 className="text-base font-bold text-gray-900 dark:text-gray-100 flex-1 leading-tight line-clamp-2 mb-2">
                        {title}
                    </h3>

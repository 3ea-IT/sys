...
export default function RoomTypeIndex({ roomTypes, search: initialSearch, status: initialStatus }) {
    const [search, setSearch] = useState(initialSearch);
    const [status, setStatus] = useState(initialStatus);

    const handleSearch = (e) => {
        e.preventDefault();
        router.get(route("admin.room-types.index"), { search, status });
    };
    ...
    const handleToggleStatus = (id) => {
        router.post(route("admin.room-types.toggle-status", id));
    };
            ...
            <div className="max-w-7xl mx-auto">
                <div className="mb-8">
                    ...
                    <p className="text-brand-secondary dark:text-gray-400">
                        Manage room types for properties
                    </p>
                </div>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-brand-border dark:border-gray-700 bg-brand-background dark:bg-gray-900">
                                ...
                                <th className="px-6 py-3 text-left text-sm font-semibold text-brand-primary dark:text-gray-200">Property</th>
                                <th className="px-6 py-3 text-left text-sm font-semibold text-brand-primary dark:text-gray-200">Price/Night</th>
                                <th className="px-6 py-3 text-left text-sm font-semibold text-brand-primary dark:text-gray-200">Rooms</th>
                        ...
                        <tbody className="divide-y divide-brand-border dark:divide-gray-700">
                            {roomTypes && roomTypes.data && roomTypes.data.length > 0 ? (
                                roomTypes.data.map((rt) => (
                                    <tr key={rt.id} className="hover:bg-brand-background dark:hover:bg-gray-700 transition-colors">
                                        <td className="px-6 py-4 text-sm text-brand-primary dark:text-gray-300 font-medium">{rt.name}</td>
                                        <td className="px-6 py-4 text-sm text-brand-secondary dark:text-gray-400">{rt.property?.name || "N/A"}</td>
                                        <td className="px-6 py-4 text-sm text-brand-primary dark:text-gray-300">₹{rt.price}</td>
                                        <td className="px-6 py-4 text-sm text-brand-primary dark:text-gray-300">{rt.room_count}</td>
                                        ...
                                        <td className="px-6 py-4 text-sm">
                                            <div className="flex gap-2">
                                                <Link
                                                    href={route("admin.room-types.show", rt.id)}
                                                ...
                                                <Link
                                                    href={route("admin.room-types.edit", rt.id)}
                                                    className="p-2 hover:bg-brand-background dark:hover:bg-gray-700 rounded transition-colors"
                                                    title="Edit"
                                                >
                ...
                {roomTypes && roomTypes.links && roomTypes.links.length > 3 && (

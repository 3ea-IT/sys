...
export default function RestaurantIndex({ restaurants, search: initialSearch, status: initialStatus }) {
    const [search, setSearch] = useState(initialSearch);
    const [status, setStatus] = useState(initialStatus);

    const handleSearch = (e) => {
        e.preventDefault();
        router.get(route("admin.restaurants.index"), { search, status });
    };
    ...
    const handleToggleStatus = (id) => {
        router.post(route("admin.restaurants.toggle-status", id));
    };
            ...
            <div className="max-w-7xl mx-auto">
                <div className="mb-8">
                    ...
                    <p className="text-brand-secondary dark:text-gray-400">
                        Manage Dining & Restaurants listings
                    </p>
                </div>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-brand-border dark:border-gray-700 bg-brand-background dark:bg-gray-900">
                                ...
                                <th className="px-6 py-3 text-left text-sm font-semibold text-brand-primary dark:text-gray-200">Cuisine</th>
                                <th className="px-6 py-3 text-left text-sm font-semibold text-brand-primary dark:text-gray-200">Price Range</th>
                        ...
                        <tbody className="divide-y divide-brand-border dark:divide-gray-700">
                            {restaurants && restaurants.data && restaurants.data.length > 0 ? (
                                restaurants.data.map((restaurant) => (
                                    <tr key={restaurant.id} className="hover:bg-brand-background dark:hover:bg-gray-700 transition-colors">
                                        <td className="px-6 py-4 text-sm text-brand-primary dark:text-gray-300 font-medium">{restaurant.name}</td>
                                        <td className="px-6 py-4 text-sm text-brand-secondary dark:text-gray-400">{restaurant.location}</td>
                                        <td className="px-6 py-4 text-sm text-brand-secondary dark:text-gray-400">{restaurant.cuisine || "N/A"}</td>
                                        <td className="px-6 py-4 text-sm text-brand-primary dark:text-gray-300 capitalize">{restaurant.price_range}</td>
                ...
                {restaurants && restaurants.links && restaurants.links.length > 3 && (

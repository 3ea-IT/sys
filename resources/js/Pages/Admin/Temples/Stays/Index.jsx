...
export default function StayIndex({
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        router.get(route("admin.stays.index"), {
            search,
            status,
        });
    };
    ...
    const handleDelete = (id) => {
        if (confirm("Are you sure you want to delete this stay?")) {
            router.delete(route("admin.stays.destroy", id));
        }
    };

    const handleToggleStatus = (id) => {
        router.post(route("admin.stays.toggle-status", id));
    };
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-brand-secondary dark:text-gray-400">
                        Manage all stay listings for temples
                    </p>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
                    <table className="w-full">
                        ...
                        <tbody className="divide-y divide-brand-border dark:divide-gray-700">
                            {stays && stays.data && stays.data.length > 0 ? (
                                stays.data.map((stay) => (
                                    <tr
                                        key={stay.id}
                ...
                {/* Pagination */}
                {stays && stays.links && stays.links.length > 3 && (
                    <div className="mt-6 flex justify-center gap-2">
                        {stays.links.map((link, index) => (
                            <Link
                                key={index}
                                href={link.url || "#"}

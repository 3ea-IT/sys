...
export default function TourismPackageIndex({
    packages,
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        router.get(route("admin.tourism-packages.index"), { search, status });
    };

    const handleDelete = (id) => {
        if (confirm("Are you sure you want to delete this package?")) {
            router.delete(route("admin.tourism-packages.destroy", id));
        }
    };

    const handleToggleStatus = (id) => {
        router.post(route("admin.tourism-packages.toggle-status", id));
    };
            ...
            <div className="max-w-7xl mx-auto">
                <div className="mb-8">
                    <h1 className="text-3xl font-bold text-brand-primary dark:text-gray-100 mb-2">
                        Tourism Packages Management
                    </h1>
                    <p className="text-brand-secondary dark:text-gray-400">
                        Manage Spiritual Tourism packages
                    </p>
                </div>

                <div className="mb-6 flex justify-end">
                    <Link
                        href={route("admin.tourism-packages.create")}
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-brand-border dark:border-gray-700 bg-brand-background dark:bg-gray-900">
                                ...
                                <th className="px-6 py-3 text-left text-sm font-semibold text-brand-primary dark:text-gray-200">Slots</th>
                        ...
                        <tbody className="divide-y divide-brand-border dark:divide-gray-700">
                            {packages && packages.data && packages.data.length > 0 ? (
                                packages.data.map((pkg) => (
                                    <tr key={pkg.id} className="hover:bg-brand-background dark:hover:bg-gray-700 transition-colors">
                                        <td className="px-6 py-4 text-sm text-brand-primary dark:text-gray-300 font-medium">{pkg.name}</td>
                                        <td className="px-6 py-4 text-sm text-brand-secondary dark:text-gray-400">{pkg.location}</td>
                                        <td className="px-6 py-4 text-sm text-brand-primary dark:text-gray-300">₹{pkg.price}</td>
                                        <td className="px-6 py-4 text-sm text-brand-primary dark:text-gray-300">{pkg.available_slots}/{pkg.capacity}</td>
                                        ...
                                        <td className="px-6 py-4 text-sm">
                                            <div className="flex gap-2">
                                                <Link
                                                    href={route("admin.tourism-packages.show", pkg.id)}
